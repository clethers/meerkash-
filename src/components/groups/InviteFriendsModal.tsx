    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <p id="invite-friends-title" className="flex items-center gap-1.5 font-semibold text-slate-900">
              <PartyPopper size={18} className="text-brand-600" /> {groupName} is ready
            </p>
            <p className="mt-1 text-sm text-slate-600">Bring people in now, or skip and do it later.</p>
          </div>
          <button onClick={close} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100" aria-label="Close">
            <X size={18} />
          </button>
        </div>
        ...
        {friends.length > 0 ? (
          ...
          <p className="mt-4 rounded-xl border border-dashed border-slate-300 px-3 py-3 text-sm text-slate-600">
